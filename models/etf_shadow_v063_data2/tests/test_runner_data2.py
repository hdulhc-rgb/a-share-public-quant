...
class Data2RunnerTests(unittest.TestCase):
    def test_dual_panel_run_keeps_research_lock_and_evidence(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(manifest["point_in_time_private_state_gate"], "PASS")
            self.assertFalse(decision["historical_validation"]["uses_live_current"])
            self.assertEqual(decision["signal_date"], index[-1].isoformat())
            self.assertIn("LATEST_COMMON_CLOSE", decision["signal_date_semantics"])
            self.assertAlmostEqual(
                float(diagnostics.loc["US_SP500", "median_abs_daily_gap"]), 0.0001
            )
            self.assertTrue((run_dir / "external_validation.json").is_file())

            alternate_current = pd.Series(
                [0.15, 0.55, 0.10, 0.10, 0.10], index=assets
            )
            alternate_run = run_research_data2(
                economic_returns=economic,
                execution_returns=execution,
                benchmark=benchmark,
                historical_anchor=benchmark,
                live_current=alternate_current,
                as_of=index[-1],
                output_root=Path(temp) / "b",
                economic_source_path=None,
                execution_source_path=None,
                economic_source_id="SYNTHETIC_ECONOMIC",
                execution_source_id="SYNTHETIC_EXECUTION",
            ...
            for filename in historical_artifacts:
                self.assertEqual(
                    ...
                    (alternate_run / filename).read_bytes(),
                    filename,
                )
            alternate_decision = json.loads(
                (alternate_run / "decision_snapshot.json").read_text(encoding="utf-8")
            )
            self.assertNotEqual(
                decision["challenger_actual_shadow_targets"],
                alternate_decision["challenger_actual_shadow_targets"],
            )
