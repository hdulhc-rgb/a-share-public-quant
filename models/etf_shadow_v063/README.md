...
- 历史验证与当期私有状态强制分离：walk-forward、CPCV 和封闭归因只使用预注册战略基准作为换手锚；最新真实持仓只参与 as-of 目标与未来执行模拟。只改变最新持仓时，全部历史验证产物必须逐字节不变。
...
`--benchmark-weights` 同时定义跟踪误差基准和历史验证的预注册换手锚；`--current-weights` 只用于最新共同收盘日生成的 live shadow target。程序不会把当前持仓回填到任何历史切片。`decision_snapshot.json` 会分别记录最新信号、最近封闭验证窗口及两类权重语义；尚无未来共同交易日时，`next_tradable_time` 保持为空，不伪造执行日期。
